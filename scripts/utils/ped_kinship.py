    ...
    def compute_node_kinship(ts, sample_nodes, max_depth=5):
        ...
        for tree in ts.trees():
            ...
            for s1, s2 in itertools.combinations(sample_nodes, 2):
                ...
                kinship[sorted_idx] += L / (2 * ts.get_sequence_length())
                
        return kinship
    
    @staticmethod
    def compute_ind_kinship(ts, individuals=None, max_depth=5):
        if individuals is None:
            individuals = TSKinship.get_sample_inds(ts)
        ...
        for tree in tqdm(ts.trees(), total=ts.num_trees):
            ...
            for s1, s2 in itertools.combinations(sample_nodes, 2):
                ...
                # Use IBD rather than tmrca since pedigree times are 
                # still unreliable. IBD tracks number of recombinations,
                # which are accurate.
                kinship[sorted_idx] += L / (4 * ts.get_sequence_length())

        print(min_tmrca, closest_pair)
        index_key = [ind.metadata.decode('utf8') for ind in individuals]
        print(index_key)
...
def sibs_plus_cousin_data():
    ...
    des.append(msprime.SimulationModelChange(pedigree_end_time, "dtwf"))

    ts = msprime.simulate(n, pedigree=pedigree, length=length, model="wf_ped",
            demographic_events=des, recombination_rate=1, Ne=100)
...
class TestKinship:
    def test_sibs_plus_counsin(self):
        ...
        PK = PedKinship.from_txt(pedfile)
        probands = PK.get_probands()
        ped_kinship = PK.compute_kinship(probands)
        print(ped_kinship)

        ts_sample_inds = TSKinship.get_sample_inds(ts)
        ts_kinship = TSKinship.compute_ind_kinship(ts, ts_sample_inds)
        print(ts_kinship)

        for j in range(len(probands)):
            for i in range(j + 1, len(probands)):
                assert(ped_kinship[i, j] == 0)
                assert(ts_kinship[i, j] == 0)
